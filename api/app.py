...
# HTML templates combined into one using triple quotes
BASE_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
    <style>
        ...
        .loading,.status-message{display:none;text-align:center;padding:20px;margin:20px 0;background:#e8f5e9;border-radius:4px;color:#4CAF50}
...
@app.route("/")
def index():
    return render_template_string(BASE_TEMPLATE, content=INDEX_CONTENT, script=INDEX_SCRIPT)

@app.route("/status")
def status():
    return render_template_string(BASE_TEMPLATE, content=STATUS_CONTENT, script=STATUS_SCRIPT)

@app.route("/gallery")
def gallery():
    return render_template_string(BASE_TEMPLATE, content=GALLERY_CONTENT, script="")
